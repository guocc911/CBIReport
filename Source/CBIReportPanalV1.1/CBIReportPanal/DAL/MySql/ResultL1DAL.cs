namespace DAL
{
    ...
    public class ResultL1DAL
    {
        public bool CheckAllResult(string keyFieldName, string barCode, string productType)
        {
            ...
            try
            {
                format = "select st120_pf,st150_pf,st160_pf,st200_pf,st230,st260,st270,st300,st320,st360,st361,st290,st410,st430,st440,st160_t,st170,st180,st210,st240_o,st250_d,st390_t1,st390_t2,st420_t1max,st420_t1min from result_line1 where {0}='{1}' order by updatetime desc limit 1";
                ...
                for (int i = 0; i < set.Tables[0].Columns.Count; i++)
                {
                    ...
                    format = "select maxvalues,minvalues from process_parameter where lineid='1' and productid='{0}' and processs='{1}'";
                    ...
                    if (set2.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (set.Tables[0].Rows[0][columnName].ToString() == string.Empty)
                        {
                            ...
                            CLog.WriteStationLog("ST440", string.Format("{0}的{1}检测结果为空", barCode, columnName));
                        }
        ...
        public void CheckResultState()
        {
            ...
            try
            {
                cmdText = "select calipercode,brakecode,bracketcode from result_line1 where calipercode is not null and brakecode is not null and bracketcode is not null and result is null and (resultdesc is null or resultdesc='')";
                ...
                if (set.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < set.Tables[0].Columns.Count; i++)
                    {
                        ...
                        cmdText = "update result_line1 set integrity=1,valid=1,result=1 where calipercode='{0}' and brakecode='{1}' and bracketcode='{2}'";
        ...
        public bool CheckStationResult(string keyFieldName, string barCode, string productType, string stationID, List<PLCDataMDL> testItemList)
        {
            ...
            try
            {
                ...
                cmdText = string.Format(cmdText + " from result_line1 where {0}='{1}' order by updatetime desc limit 1", keyFieldName, barCode);
                set = MySqlDBHelper.ExecuteDataSet(MySqlDBHelper.Conn, CommandType.Text, cmdText, null);
                if (set.Tables[0].Rows.Count <= 0)
                {
                    return false;
        ...
        public bool Exist(string keyName, string keyValue)
        {
            ...
            try
            {
                format = "select count({0}) from result_line1 where {0}='{1}'";
        ...
        public string GetBracketByCaliperCode(string caliperCode)
        {
            try
            {
                object obj2 = null;
                string format = string.Empty;
                format = "select bracketcode from result_line1 where calipercode='{0}' order by updatetime desc limit 1";
        ...
        public string GetBracketByCaliperOrBrakeCode(string caliperCode, string brakeCode)
        {
            try
            {
                object obj2 = null;
                string format = string.Empty;
                format = "select bracketcode from result_line1 where calipercode='{0}' or brakecode='{1}' order by updatetime desc limit 1";
        ...
        public string GetBrakeByCaliperCode(string caliperCode)
        {
            try
            {
                object obj2 = null;
                string format = string.Empty;
                format = "select brakecode from result_line1 where length(brakecode)>0 and brakecode is not null and calipercode='{0}' order by updatetime desc limit 1";
                format = string.Format(format, caliperCode);
                obj2 = MySqlDBHelper.ExecuteScalar(MySqlDBHelper.Conn, CommandType.Text, format, null);
                if (obj2 != null)
                {
        ...
        public string GetCaliperByBrakeCode(string brakeCode)
        {
            try
            {
                object obj2 = null;
                string format = string.Empty;
                format = "select calipercode from result_line1 where length(calipercode)>0 and calipercode is not null and brakecode='{0}' order by updatetime desc limit 1";
        ...
        public string GetFieldValueByKeyCode(string fieldName, string keyName, string keyValue)
        {
            try
            {
                object obj2 = null;
                string format = string.Empty;
                format = "select {0} from result_line1 where {1}='{2}' order by tid desc limit 1";
        ...
        public int InsertCode(string codeName, string codeValue)
        {
            ...
            try
            {
                format = "insert into result_line1({0},createtime) values ('{1}',now())";
        ...
        public int MoveBrakeDataToCaliper(string brakeCode, string caliperCode)
        {
            ...
            try
            {
                format = "select tid,st300,st320,st332,st350,st360,st361,st290,trialproduce from result_line1 where (length(calipercode)<=0 or calipercode is null) and brakecode='{0}' order by updatetime desc limit 1";
                format = string.Format(format, brakeCode);
                set = MySqlDBHelper.ExecuteDataSet(MySqlDBHelper.Conn, CommandType.Text, format, null);
                if (set.Tables[0].Rows.Count <= 0)
                {
                ...
                if (num9 == 1)
                {
                    format = "update result_line1 set brakecode=@brakecode,st300=@st300,st320=@st320,st332=@st332,st350=@st350,st360=@st360,st361=@st361,st290=@st290,trialproduce=1,updatetime = now() where calipercode=@calipercode";
                }
                else
                {
                    format = "update result_line1 set brakecode=@brakecode,st300=@st300,st320=@st320,st332=@st332,st350=@st350,st360=@st360,st361=@st361,st290=@st290,updatetime = now() where calipercode=@calipercode";
                }
                num10 = MySqlDBHelper.ExecuteNonQuery(MySqlDBHelper.Conn, CommandType.Text, format, commandParameters);
                if (num10 > 0)
                {
                    format = "delete from result_line1 where tid={0}";
        ...
        public int UpdateByField(string fieldName, object fieldValue, string keyName, string keyValue)
        {
            ...
            try
            {
                ...
                {
                    format = "update result_line1 set {0}='{1}',updatetime = now(),uploadsign = 0 where {2}='{3}'";
                }
                else
                {
        ...
        public int UpdateByField(string fieldName, string fieldValue, int valueType, string keyName, string keyValue)
        {
            ...
            try
            {
                if (valueType == 2)
                {
                    format = "update result_line1 set {0}='{1}',updatetime = now() where {2}='{3}'";
                }
                else if (valueType == 1)
                {
                    format = "update result_line1 set {0}={1},updatetime = now() where {2}='{3}'";
        ...
        public bool UpdateProductNameByKeyCode(string keyFieldName, string barCode, string productType)
        {
            ...
            try
            {
                ...
                format = "update result_line1 set productname='{0}' where {1}='{2}'";
        ...
        public int UpdateResultByKeyCode(string keyFieldName, string barCode, int integrity, int valid, int result, string resultDesc)
        {
            ...
            try
            {
                format = "update result_line1 set integrity={0},valid={1},result={2},resultdesc='{3}' where {4}='{5}'";
