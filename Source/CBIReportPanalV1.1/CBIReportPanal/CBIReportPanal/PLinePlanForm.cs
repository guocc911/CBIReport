...
namespace CBIReportPanal
{
    public partial class PLinePlanForm : Form
    {
        ...
        {
            //判断ID是否加载
            if (BaseVariable.LineID == null || Int32.TryParse(BaseVariable.LineID, out lineID) == false)
            {
                MessageBox.Show("加载配置文件失败,请检查后重试!");
                return;
            }
        ...
        private void PLinePlanForm_Load(object sender, EventArgs e)
        {
           ...
           // InitalCombox();
            ...
            this.lbPLineID.Text = lineID.ToString() + "号产线";
                ...
                foreach (DataRow itemRow in table.Rows)
                {
                    foreach(TimeRange item in rangs)
                    {
                        ...
                        if(item.IsInFullRange(ptime1,ptime2))
                        {
                            ...
                            app.Subject = string.Format("型号：{0}  预计产量:{1}  备注:{2}",
                                                         mdl.BreakeID.ToString(), 
                                                         mdl.PlanNum.ToString(),
                                                         mdl.Remark.Trim('0'));
                                                        // mdl.Actual_num.ToString());
